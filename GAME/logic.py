...
from GAME.save.save import save_file, load_file, dict_to_sprite_list, sprite_list_to_dict
import GAME.entity as entity
from GAME.entity.item_cls.swords.fist import Fist
from GAME.inventory.inventory import Inventory
from GAME.map.map1.map1 import MapEngine
...
class GameView(arcade.View):
    def __init__(self, game = None, save_file = ""):
        ...
        # Map
        self.map1 = MapEngine(self.player)
        self.map1.setup()
    ...
    def on_draw(self):
        ...
        # On dessine le perso en premier pr donner une impression de profondeur qd il passe derrière un arbre
    ...
    def on_show_view(self):
        ...
        if len(self.enemy_list) < self.number_max_enemy:
            self.create_enemy(number_of_enemy = self.number_max_enemy - len(self.enemy_list),
                              area_of_spawn= ((self.player.center_x - 2500, self.player.center_y - 2500), (self.player.center_x + 2500, self.player.center_y + 2500)))
        if self.map1.can_attack:
            self.choose_best_sword()
...
# Fonction non présentes de base dans arcade
    ...
    # Differents menus
    def open_pause(self):
        from GAME.menu.menu_pause.menu_pause import MenuPause
        self.game.switch_scene(MenuPause(self.game, self.save_file, gameview=self))

    def open_inventory(self):
        from GAME.menu.menu_inventory.menu_inventaire import InventoryMenu
    ...
    def open_gameover(self):
        from GAME.menu.menu_gameover.menu_gameover import MenuGameover
    ...
    def open_craft(self):
        from GAME.menu.menu_craft.menu_craft import MenuCraft
